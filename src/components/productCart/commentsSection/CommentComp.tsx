  ...
  return (
    <>
      <div style={containerStyle}>
        <div className='parentComment flex flex-col border-b-2 pt-[15px] pb-[15px]'>
          ...
          {comment.inputImg.length > 0 ? (
            <Image src={comment.inputImg} width={200} height={200} alt='inputImg' />
          ) : (
            ''
          )}
          ...
          <div className='parentComment__footer flex flex-row justify-between text-[#6C6C6C]'>
            ...
            <div className='flex flex-row items-center justify-between'>
              ...
              <p className='hover:cursor-pointer' onClick={handleReplyClick}>
                Відповісти
              </p>
            </div>
          </div>
          ...
          <div className='flex flex-row justify-between gap-6 mb-[69px]'>
            <div className='relative w-full flex items-center'>
              <input
                ...
                onChange={e => handleRequestFile(e)}
              />
              <input
                type='text'
                placeholder='Написати відгук'
                className='w-full bg-transparent border-b-[1px] border-[#C1C1C1] pr-[25px]'
                value={replyText}
              ...
              <label htmlFor='uploadFile'>
                <Image
                  src={clipGrey}
                  alt='clipGrey'
                  className='absolute top-[25%] right-0 hover:cursor-pointer'
                />
              </label>
            </div>
            <button
              className='flex items-center justify-center h-[36px] w-[105px] pt-[6px] pr-[14px] pb-[7px] pl-[14px]  bg-[#B9FF61] rounded-[30px] text-center'
          ...
          <div className='childComment'>
            {comment.reply.map((reply, index) => (
              <CommentComp
                ...
                comment={reply}
                depth={depth + 1}
                onAddReply={(text, parentComment) => onAddReply(text, parentComment)}
              />
            ))}
          </div>
        )}
